...
| radarr | `lscr.io/linuxserver/radarr:6.3.0` | 7878 | `http://radarr.<DOMAIN>` |
| bazarr | `lscr.io/linuxserver/bazarr:1.6.0` | 6767 | `http://bazarr.<DOMAIN>` |
...
2. For the media stack, copy the template and fill in the environment variables:
   ...
   | `SERVICE_URL_SONARR_8989` etc. | Coolify routing URLs, e.g. `http://sonarr.<DOMAIN>:8989` |
...
4. Set `MEDIA_DIR`, `COOLIFY_NETWORK`, and the `SERVICE_URL_*` values in the Coolify UI environment variables (plus Domains for gluetun), and ensure the media directories exist on the host, e.g.:
